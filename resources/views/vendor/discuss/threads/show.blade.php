...
@section('content')
    <section>
        <div class="container">
            <thread :thread="{{$thread}}" :channels="{{$channels}}" :user="{{$user ?? '{}'}}" :subscribed="{{ $subscribed  }}" :imagefield="'{{ config('discuss.user_image_field') }}'" :defaultimage="'{{ config('discuss.default_image') }}'">
